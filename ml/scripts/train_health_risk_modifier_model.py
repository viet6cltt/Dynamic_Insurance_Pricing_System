#!/usr/bin/env python3
"""Train the Health Risk Modifier model for hybrid pricing."""
...
NUMERIC_FEATURES = ["age", "bmi", "children", "blood_pressure"]
...
HEALTH_RISK_FACTOR_CLAMP = {"min": 0.75, "max": 2.5}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        default="data/health_insurance_cost_and_risk_dataset.csv",
        help="Old medical cost CSV path, relative to the repo root by default.",
    )
    ...
    parser.add_argument(
        ...
        default="HealthRiskModifierModel",
...
def load_training_frame(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Health risk modifier dataset is missing columns: {missing}")
    ...
    if out.empty:
        raise ValueError("No positive target rows remain after cleaning.")
    if dropped:
        print(f"Dropped {dropped} rows with missing/non-positive charges.")
    return out
...
def build_models(sklearn: dict[str, Any], seed: int, include_xgboost: bool = True) -> dict[str, Any]:
    GammaRegressor = sklearn["GammaRegressor"]
    TweedieRegressor = sklearn["TweedieRegressor"]

    models: dict[str, Any] = {
        "gamma_regressor": GammaRegressor(alpha=0.1, max_iter=1000),
        "tweedie_regressor": TweedieRegressor(power=1.5, alpha=0.1, link="log", max_iter=1000),
    }
    ...
    try:
        ...
        print(f"XGBoost unavailable; continuing with GLM-family models only: {exc}")
    else:
...
def normalized_gini(y_true: pd.Series | np.ndarray, y_pred: np.ndarray) -> float:
    ...
    if perfect == 0:
        return 0.0
    return float(gini(actual, pred) / perfect)
...
def evaluate(y_true: pd.Series, y_pred: np.ndarray, sklearn: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "MAPE": float(mean_absolute_percentage_error(y_true, clipped)),
...
def cross_validate_model(
    ...
    folds = 5 if len(x_train) >= 5 else len(x_train)
    scoring = {
    ...
    scores = cross_validate(pipeline, x_train, y_train, cv=cv, scoring=scoring, n_jobs=None)
    return {
...
def predict_baseline_frame(frame: pd.DataFrame) -> pd.DataFrame:
    baseline = frame.copy()
...
def health_risk_factor(pipeline: Any, profile: pd.DataFrame) -> dict[str, float]:
    ...
    baseline_frame = predict_baseline_frame(profile)
    baseline = float(max(pipeline.predict(baseline_frame)[0], 1e-6))
...
def write_report(path: Path, metadata: dict[str, Any], comparison: pd.DataFrame) -> None:
    ...
    lines = [
        "# Health Risk Modifier Model Report",
        "",
        "## Purpose",
        "",
        "- Model 2 predicts expected medical cost from the old medical cost dataset.",
        "- Its prediction is converted into a relative `healthRiskFactor`.",
        "- This is not a final premium model and does not use portfolio premium or claim fields.",
        "",
        "## Dataset And Features",
        "",
        f"- Input dataset: `{metadata['inputDataset']}`",
        f"- Target: `{TARGET_COLUMN}` interpreted as historical medical cost.",
        ...
        "## Baseline Conversion",
        "",
        "- `predictedHealthCost = HealthRiskModel(age, sex, bmi, smoker, children, blood_pressure, exercise_frequency, pre_existing_condition, occupation_risk)`",
        "- `baselineHealthCost = HealthRiskModel(age, sex, bmi=22.0, smoker=no, same children, blood_pressure=120.0, exercise_frequency=daily, pre_existing_condition=false, occupation_risk=low)`",
        "- `healthRiskFactor = clamp(predictedHealthCost / baselineHealthCost, 0.75, 2.50)`",
        "",
        "## Selected Model",
        "",
        f"- Model version: `{metadata['modelVersion']}`",
        f"- Selected model: `{selected}`",
        f"- Selection metric: lowest holdout RMSE, with CV RMSE as supporting evidence.",
        ...
        "## Guardrails",
        "",
        "- Do not add this prediction to Model 1 output.",
        "- Do not merge this dataset row-by-row with the portfolio dataset.",
        "- Chronic disease/pre-existing condition is handled by transparent underwriting rules in MVP.",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def log_to_mlflow(
    *,
    ...
    with mlflow.start_run(run_name=metadata["modelVersion"]):
        mlflow.set_tags(
            {
                "model_role": "health_risk_modifier",
        ...
        mlflow.log_params(
            {
                ...
                "clamp_min": metadata["healthRiskFactorFormula"]["clamp"]["min"],
                "clamp_max": metadata["healthRiskFactorFormula"]["clamp"]["max"],
            }
        )
...
def main() -> int:
    ...
    df = load_training_frame(Path(args.input))
    ...
    sample_profile = x_test.iloc[[0]].copy()
    sample_conversion = health_risk_factor(best_pipeline, sample_profile)
    training_date = datetime.now(timezone.utc).replace(microsecond=0).isoformat()
    model_version = f"health-risk-modifier-v1-{selected_model}"

    metadata = {
        "modelVersion": model_version,
        "modelPurpose": "health risk modifier, not final premium model",
        "datasetRole": "old medical cost dataset",
        "selectedModel": selected_model,
        "trainingDate": training_date,
        "inputDataset": args.input,
        "rowCount": int(len(df)),
        "targetColumn": TARGET_COLUMN,
        "targetMeaning": "historical medical cost used to learn relative health risk effects",
        "featureList": MODEL_FEATURES,
        "numericFeatures": NUMERIC_FEATURES,
        "categoricalFeatures": CATEGORICAL_FEATURES,
        "transformedFeatureList": transformed_names,
        "excludedColumns": EXCLUDED_COLUMNS,
        "baselineHealthProfile": BASELINE_HEALTH_PROFILE,
        "healthRiskFactorFormula": {
            "predictedHealthCost": "HealthRiskModel(age, sex, bmi, smoker, children, blood_pressure, exercise_frequency, pre_existing_condition, occupation_risk)",
            "baselineHealthCost": (
                "HealthRiskModel(age, sex, bmi=22.0, smoker=no, same children, blood_pressure=120.0, exercise_frequency=daily, pre_existing_condition=false, occupation_risk=low)"
            ),
            "healthRiskFactor": "predictedHealthCost / baselineHealthCost",
            "clamp": HEALTH_RISK_FACTOR_CLAMP,
        },
        "modelMetrics": metrics,
        "sampleHealthRiskFactorCalculation": sample_conversion,
        "evaluationDesign": {
            "testSplit": "80/20",
            "testSize": args.test_size,
            "randomState": args.seed,
            "crossValidation": "5-fold KFold on training set",
            "selectionMetric": "lowest holdout RMSE",
        },
        "pythonVersion": platform.python_version(),
        "noFinalPremiumInModel": True,
        "noPortfolioDatasetMerge": True,
        "chronicDiseasePolicy": (
            "Pre-existing conditions are directly processed as a feature by the ML model."
        ),
    }
    ...
    comparison_path = reports_dir / "health_risk_modifier_comparison.csv"
    report_path = reports_dir / "health_risk_modifier_model_report.md"
